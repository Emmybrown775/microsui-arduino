...
void compact_ed25519_sign(
    ...
    edsign_sign(signature, private_key + __PUBLIC_KEY_OFFSET, private_key, (const uint8_t*)message, msg_length);
}
...
bool compact_ed25519_verify(
    ...
    return edsign_verify(signature, public_key, (const uint8_t*) message, msg_length) != 0;
}
#endif
